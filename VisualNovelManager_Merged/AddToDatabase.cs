    ...
    class AddToDatabase
    {
        ...
        internal  void AddNovelData()
        {
            ...
            try
            {
                var cmd =
                    new SqlCeCommand(
                        "INSERT INTO NovelPath(Novel, ExePath, VnId, IconPath, LastPlayed, PlayTime) VALUES(@Novel, @ExePath, @VnId, @IconPath, @LastPlayed, @PlayTime)",
                        con) { Connection = con };
        ...
        internal  void AddVnApi()
        {
            ...
            try
            {
                var cmd =
                    new SqlCeCommand(
                        "INSERT INTO VnAPI(VnId, title, original, released, languages, orig_lang, platforms, aliases, length, description, links, image, image_nsfw, popularity, rating, screenshots) " +
                        "VALUES(@VnId, @title, @original, @released, @languages, @orig_lang, @platforms, @aliases, @length, @description, @links, @image, @image_nsfw, @popularity, @rating, @screenshots)",
                        con) { Connection = con };
                cmd.Parameters.AddWithValue("@VnId", CheckForDbNull(_basicItem[0].id));
        ...
        internal  void AddCharacterApi()
        {
            ...
            try
            {
                ...
                for (int j = 0; j < _characterCount; j++)
                {
                    ...
                    SqlCeCommand cmd = new SqlCeCommand("INSERT INTO CharacterAPI(VnId, CharacterId, Name, Original, Gender, BloodType, Birthday, Aliases, Description, Image, Bust, Waist, Hip, Height, Weight, vns)" +
