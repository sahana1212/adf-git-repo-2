    ...
    public class UserVnListViewModel: INotifyPropertyChanged
    {
        ...
        private async Task<List<string>> GetVnListVoteList(string username, SecureString passwordSecure)
        {
            try
            {
                ...
                const string connectionString = @"Data Source=|DataDirectory|\Database.db";
                var con = new SQLiteConnection(connectionString);
                con.Open();
                SQLiteCommand vnlistsqlcmd = new SQLiteCommand("SELECT * FROM VnList", con);
                SQLiteDataReader reader = vnlistsqlcmd.ExecuteReader();
                while (reader.Read())
                {
                ...
                foreach (var id in SqlVnList)
                {
                    ...
                    {
                        var deleteVncmd = new SQLiteCommand("DELETE FROM VnList WHERE VnId=@VnId", con);
                ...
                var cmd = new SQLiteCommand("INSERT INTO VnList(VnId, Title, NSFW, Vote, Status, Note) VALUES(@VnId, @Title, @NSFW, @Vote, @Status, @Note)", con) { Connection = con };
                            ...
                            if (page == 1)
                            {
                                for (int i = 0; i < _basicItem.Count; i++)
                                {
                                    ...
                                    else
                                    {
                                        //var con = new SQLiteConnection(connectionString);
                                        con.Open();
                                        var sqlnamecmd = new SQLiteCommand("SELECT * FROM VnList WHERE VnId=@VnId", con);
                                        sqlnamecmd.Parameters.AddWithValue("@VnId", _basicItem[i].id);
                                        SQLiteDataReader nameReader = sqlnamecmd.ExecuteReader();
                                    ...
                                    foreach (var listItem in sqlList)//updates the entries
                                    {
                                        if (listItem[0].Equals(_basicItem[i].id.ToString()))
                                        {
                                            ...
                                            var cmd = new SQLiteCommand("UPDATE VnList SET NSFW=@NSFW, Status=@Status, Note=@Note, Vote=@Vote WHERE VnId=@VnId", con) { Connection = con };
